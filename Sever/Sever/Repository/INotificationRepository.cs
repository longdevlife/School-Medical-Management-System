using Microsoft.EntityFrameworkCore;
using Sever.Context;
using Sever.Model;

namespace Sever.Repository.Interfaces
{
    public interface INotificationRepository
    {
        Task AddNotificationAsync(Notify notification);
        Task<List<string>> GetParentIdsByMedicalEventAsync(string medicalEventID);
        Task<bool> CheckParentExistsAsync(string parentID);
        Task<Form> GetFormByIdAsync(string formId);
        Task CreateFormAsync(Form form);
        Task<string> GenerateNotifyIDAsync();
    }
    public class NotificationRepository : INotificationRepository
    {
        private readonly DataContext _context;

        public NotificationRepository(DataContext context)
        {
            _context = context;
        }

        public async Task AddNotificationAsync(Notify notification)
        {
            await _context.Notify.AddAsync(notification);
            await _context.SaveChangesAsync();
        }
        public async Task<List<string>> GetParentIdsByMedicalEventAsync(string medicalEventID)
        {
            var studentIds = await _context.MedicalEventDetail
                .Where(d => d.MedicalEventID == medicalEventID)
                .Select(d => d.StudentID)
                .ToListAsync();

            if (!studentIds.Any()) return new List<string>();

            var parentIds = await _context.Users
                .Where(u => u.RoleID == 1 && u.StudentProfile.Any(sp => studentIds.Contains(sp.StudentID)))
                .Select(u => u.UserID)
                .ToListAsync();

            return parentIds;
        }

        public async Task<bool> CheckParentExistsAsync(string parentID)
        {
            return await _context.Users.AnyAsync(u => u.UserID == parentID && u.RoleID == 1);
        }

        public async Task CreateFormAsync(Form form)
        {
            _context.Form.Add(form);
            await _context.SaveChangesAsync();
        }

        public async Task<string> GenerateNotifyIDAsync()
        {
            var latest = await _context.Notify.OrderByDescending(n => n.NotifyID).FirstOrDefaultAsync();
            if (latest != null && latest.NotifyID.StartsWith("N"))
            {
                var numberPart = latest.NotifyID.Substring(1);
                if (int.TryParse(numberPart, out int number))
                    return $"N{(number + 1):D3}";
            }
            return "N001";
        }

        public async Task<Form> GetFormByIdAsync(string formId)
        {
            return await _context.Form.FirstOrDefaultAsync(f => f.FormID == formId);
        }
    }
}
